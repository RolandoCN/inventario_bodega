...
<body>
    <div class="ticket centrado">
        ...
        <table width="{{$tabla_dos}}" style="padding-bottom:2px !important; margin-left:{{$mar_dos}};margin-right:{{$mar_dos_derecha}};margin-top:10px">
            <thead>
                <tr class="centrado" style="font-size: 10px !important">
                    <th >ITEM</th>
                    <th >CANT</th>
                    ...
                    @foreach($comprobante->detalle  as $e=>$dato)
                        @if($dato->pedido->cantidad_entregada>0)
                            ...
                            <tr style="font-size: 10px !important;border-top: 1px solid black">
                                <td width="60%"> 
                                    <p style="line-height:7px">{{$dato->item->nombre}} {{$dato->item->concentra}} {{$dato->item->forma}} {{$dato->item->presentacion}}</p>
                                 </td>
    ...
    @if($comprobante->paciente)
        <div style="margin-top:30px;margin-bottom:30px; margin-left:30px;margin-right:30px; font-size:710pxx !important">
            <hr>
            <p class="encabezado1" style="font-size:10px !important"><b>RECIBI CONFORME</b></p>
            <p class="encabezado1_" style="line-height: 10px">
                {{$comprobante->paciente->apellido1}} {{$comprobante->paciente->nombre1}}
            </p>
            <p class="encabezado1_" style="line-height: 10px">
                {{$comprobante->paciente->apellido1}} 
            </p>
        </div>
    @else
        <div style="margin-top:30px;margin-bottom:30px; margin-left:30px;margin-right:30px; font-size:710pxx !important">
