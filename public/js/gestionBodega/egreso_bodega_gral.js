...
function tipoEgreso(){
    ...
    if(tipo==""){return}
    if(tipo=="Externo"){
        $('#retirado_seccion').show()
        limpiarRetirado()
    }else{
        $('#retirado_seccion').hide()
        limpiarRetirado()
    }
...
function consultaRc(){
    ...
    if(identificacion==""){
        alertNotificar("Ingrese el numero o codigo de identificacion","error");
        $('#identificacion').focus()
        return
    }

    //si es cedula validamos que sea numero y 10 digitos
    if(isNaN(identificacion)){
        alertNotificar("Ingrese solo numero, para el tipo de identificacion cedula","error");
        return
    }
    if(identificacion.length!=10){
        alertNotificar("El numero de cedula debe tener 10 digitos","error");
        return
    }

    vistacargando("m","Espere por favor")
    $.get("http://172.10.40.220/ws/webservice/test.php?cedula="+identificacion, function(data){
        ...
        if(dataRC.respuesta.CodigoMensaje!=="000"){
            alertNotificar("No se pudo traer la informacion del registro civil, intenlo nuevamente", "error")
            $('#nombre_retira').prop("readonly", false)
            return
        }

        var nombres=dataRC.respuesta.Ciudadano.Nombre
      
        $('#nombre_retira').val(nombres)
        $('#nombre_retira').prop("readonly", true)
...
$("#form_medicina").submit(function(e){
    ...
    }else if(bodega_selecc==8 || bodega_selecc==19 || bodega_selecc==13 || bodega_selecc==23 || bodega_selecc==14 || bodega_selecc==24){
        url_busqueda="listado-lab-filtra/"+txt_item+"/"+bodega_selecc; 
    }else if(bodega_selecc==30) {
        url_busqueda="listado-proteccion-lote/"+txt_item+"/"+bodega_selecc; 
    }
    else {
        url_busqueda="listado-items-lote/"+txt_item+"/"+bodega_selecc; 
    }
...
$("#form_egreso_bodega").submit(function(e){
    ...
    if(AccionForm=="R"){
        tipo="POST"
        url_form="guardar-egreso-bodega"
    }else{
        tipo="PUT"
