...
function listarMedGral(bodega=2){
...
function dataGlobalMd(bodega=2,txt){
    let cmb_bodega=bodega
    let cmb_tipo="BODEGA"
    BodeSelecc=bodega
    volverBusqueda()
			...
			$.each(data.resultado,function(i, item){
                ...
                $('#tabla_inventario_global_md').append(`<tr >
                                            ...
                                            <td style="width:20%; text-align:center; vertical-align:middle">
                                                ...
                                                <button type="button" ${disabled} class="btn btn-info btn-xs" onclick="verAccesoEnf('${item.id_item}', '${cmb_bodega}')">Parametros Lic</button>
...
function verAccesoEnf(id, bodega, abiertaModal=null){
    ...
    $.get('acceso-item-enf/'+id+'/'+bodega, function(data){
		...
		if(data.error==false){
            ...
            globalThis.InsumoSeleccionado=id
            if(abiertaModal!="S"){
                $('#modal_busqueda_acceso').modal('show')
                $('#insumo_parametro').html(nombre_insumo)
                $('.modal-title').html('ACCESO INSUMO LICENCIADOS/AS Y LIDERES AREA')
            }
		}
    })
...
function verAcceso(id, bodega, abiertaModal=null){
    ...
    $.get('acceso-item/'+id+'/'+bodega, function(data){
		...
		if(data.error==false){
            ...
            if(abiertaModal!="S"){
                $('#modal_busqueda_acceso').modal('show')
                $('#insumo_parametro').html(nombre_insumo)
                $('.modal-title').html('ACCESO INSUMO MEDICOS')
            }
		}
    })
...
function accionAccesoEnf(id){
    ...
    if( $('#check_'+id).is(':checked') ){
        ...
        AggQuitarAccesoInsumo(id,'A','L')
    } else {
        // mandamos a quitar
        AggQuitarAccesoInsumo(id,'Q','L')
    }
}
...
function accionAcceso(id){
    ...
    if( $('#check_'+id).is(':checked') ){
        // mandamos a guardar ese menu al perfil
        AggQuitarAccesoInsumo(id,'A','M')
    } else {
        // mandamos a quitar
        AggQuitarAccesoInsumo(id,'Q','M')
    }
}

function AggQuitarAccesoInsumo(idarea_esp, tipo, lugar){
    vistacargando("m","Espere por favor")
    $.get("insumo-por-area/"+idarea_esp+"/"+tipo+"/"+InsumoSeleccionado, function(data){
        vistacargando("")
        if(data.error==true){
            if(tipo=='A'){
        ...
        if(lugar=='L'){
            verAccesoEnf(InsumoSeleccionado, 2,'S')
        }else{
            verAcceso(InsumoSeleccionado, 2,'S')
        }
