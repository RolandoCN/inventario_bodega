function buscarPedidos(){
    ...
    $.get('filtra-devolver-farmacia-bod/'+fecha_inicial+'/'+fecha_final, function(data){
        console.log(data)
			...
			$.each(data.resultado,function(i, item){
				...
				$('#tabla_pedido').append(`<tr>
                                                ...
                                                <td style="width:10%; text-align:left; vertical-align:middle">
                                                    ...
                                                    <button type="button" class="btn btn-xs btn-primary" onclick="Detalle('${item.idcomprobante}','${item.idbodega}','${item.descripcion}','${item.secuencial}','${item.solicita}','${item.area}','${item.fecha_hora}','${item.nombre_bodega}','${item.id_usuario_ingresa}')">Detalle</button>
...
function imprimir(id, bodega){
    ...
    $.get("reporte-devolucion-farm-bodega/"+id+"/"+bodega, function(data){
        console.log(data)
        vistacargando("")
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
...
function Detalle(idcomprobante, idbodega, desc, secue, solicita, area, fecha, bodega, idusuaSoli){
    ...
    vistacargando("m","Espere por favor")
    $.get("detalle-pedidos/"+idcomprobante+"/"+idbodega, function(data){
        console.log(data)
        vistacargando("")
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
        ...
        $.each(data.resultado,function(i, item){
            ...
            $('#tabla_detalle_pedido').append(`<tr>
                                            ...
                                            <td style="width:15%; text-align:left; vertical-align:middle">
                                                ...
                                                <input type="number"id="class_cantidad_validada-${item.iddetalle}" style="width:100% !important;text-align:right" name="cantidad_validada[]" onkeyup="validar_cantidad(this,${item.iddetalle})" onblur="validar_cantidad(this,${item.iddetalle})" onclick="validar_cantidad_click(this,${item.iddetalle})" >
                                            ...
                                            <td style="width:5%; text-align:left; vertical-align:middle">
                                               <button disabled type="button" class="btn btn-xs btn-info" onclick="verHistorial('${idusuaSoli}','${item.iditem}','${item.nombre_item}','${solicita}')">
...
function validar(){
    ...
    //9=>tics
    //10=>lenceria
    if(bodegaSelecc==4 || bodegaSelecc==3 || bodegaSelecc==30 || bodegaSelecc==9 || bodegaSelecc==10){
        url_aprobar="validar-pedido-solicitado-area"
    }else{
        url_aprobar="validar-devolucion-farmacia"
    }
    
    
    swal({
        title: "¿Desea reingresar los items a bodega?",
        type: "warning",
        showCancelButton: true,
        confirmButtonClass: "btn-danger",
        confirmButtonText: "Si, continuar",
