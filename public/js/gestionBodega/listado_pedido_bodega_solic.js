...
function Actualizar(id, bodega){
    ...
    $.get("actualizar-pedido/"+id+"/"+bodega, function(data){
        vistacargando("")     
        console.log(data)     
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
...
function Detalle(idcomprobante, idbodega, desc, secue, solicita, area, fecha, bodega){
    ...
    vistacargando("m","Espere por favor")
    $.get("detalle-pedidos-sol/"+idcomprobante+"/"+idbodega, function(data){
        console.log(data)
        vistacargando("")
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
...
function tecla_cantidad(e, id){
    ...
    var cant_solicitada=$('#class_cantidad_pedida'+id).val()
    ...
    if(parseFloat(cant_validada) > parseFloat(cant_solicitada)){
        alertNotificar("La cantidad validada debe ser menor o igual a "+cant_solicitada, "error")
        $('#class_cantidad_validada-'+id).val('')
        $('#class_cantidad_validada-'+id).focus()
        return
    }
...
function llenarTabla(data){
    ...
    CantidadItem=1000
    $.each(data.resultado.detalle_item,function(i,item){
        
        let lote=""
        if(item.pedido.lote==null || item.pedido.lote=="null"){}
        else{
            lote=item.pedido.lote
        }
        let total_fila_=0
        ...
        $('#tb_listaMedicamento').append(`<tr id="medicamentos_${item.id_item}">
            ...
            <td width="27%" class="centrado">
                ...
                <input type="hidden" name="idbodega_producto[]" id="idbodega_producto${item.id_item}" value="${item.id_item}">
            ${item.itemlab.descri}
            </td> 
            ...
            <td width="8%" class="centrado">
                <input type="number"id="class_precio_${item.id_item}" step=""0.01" style="width:100% !important;text-align:right" name="precio[]"  onblur="validar_precio(this,'${item.id_item}')" value="${item.precio}" readonly >
            </td>  

        
            <td width="12%" class="centrado">
                <input type="date"  id="class_fecha_elab_${item.id_item}" style="width:100% !important;text-align:right" name="fecha_elab_[]")  value="${item.pedido.fecha_elabora}" readonly>
            </td>  

            <td width="12%" class="centrado">
                <input type="date"id="class_fecha_caduc_${item.id_item}"  style="width:100% !important;text-align:right" name="fecha_caduc[]" value="${item.pedido.fecha_caducidad}" readonly>
            </td>  


            <td width="8%" class="centrado">
                <input type="text"id="class_lote_${item.id_item}"  style="width:100% !important;text-align:right" name="lote[]"  value="${lote}" readonly>
            </td>

            <td width="8%" class="centrado">
                <input type="text"id="class_reg_sani_${item.id_item}" style="width:100% !important;text-align:right" name="reg_sani[]" value="" readonly>
            </td>

            <td width="9%" align="right" class="centrado">
                <input type="hidden" readonly id="class_total_${item.id_item}"  style="width:100% !important;text-align:right" name="total[]" value="${total_fila_}">
                <span style="text-align:right" id="total_span_id_${item.id_item}">${total_fila_}</span>
            </td>  
...
function llenarTablaProtecc(data){
    ...
    let bodega=data.resultado.idbodega
    CantidadItem=1000
    $.each(data.resultado.detalle_proteccion,function(i,item){
...
function llenarTablaLab(data){
    ...
    let bodega=data.resultado.idbodega
    CantidadItem=1000
    $.each(data.resultado.detalle_lab,function(i,item){
        let total_fila_=0
        total_fila_=item.cantidad *item.precio
        ...
        $('#tb_listaMedicamento').append(`<tr id="medicamentos_${item.id_item}">
            ...
            <td width="27%" class="centrado">
                ...
                <input type="hidden" name="idbodega_producto[]" id="idbodega_producto${item.id_item}" value="${item.pedido.idbodpro}">
...
function llenarTablaInsumo(data){
    ...
    let bodega=data.resultado.idbodega
    CantidadItem=1000
    $.each(data.resultado.detalle_insumo,function(i,item){
        let total_fila_=0
        total_fila_=item.cantidad *item.precio
        total_fila_=total_fila_.toFixed(2)
...
$("#form_pedido_bodega").submit(function(e){
    ...
    if(AccionForm=="R"){
        ...
        tipo="PUT"
        if(bod==8 || bod==13 || bod==14 || bod==19 || bod==23 || bod==24 || bod==18 || bod==17 || bod==2){
            url_form="actualizar-pedido-bodega-farm/"+idComprobanteActualizar
        }else if(bod==22 || bod==25 || bod==26 || bod==27 || bod==28 || bod==29){
            //pedidos a bod d farmacia desde lab
            url_form="actualizar-pedido-bodega-farm-laborat/"+idComprobanteActualizar
        }else if(bod==18 || bod==21 || bod==7 || bod==2 ){
            //pedidos a bod insumo d farmacia desde lab
            url_form="actualizar-pedido-bodega-farm-laborat/"+idComprobanteActualizar
        }else if(bod==1 || bod==17 || bod==20 || bod==6){
            //pedidos a bod medicamentos d farmacia desde lab
            url_form="actualizar-pedido-bodega-farm-laborat/"+idComprobanteActualizar
        }else{
            url_form="actualizar-pedido-bodega-area/"+idComprobanteActualizar
        }
