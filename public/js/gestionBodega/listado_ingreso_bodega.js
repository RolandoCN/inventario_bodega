...
function buscarProveedor(){
    $('#proveedor_cmb').select2({
        placeholder: 'Seleccione una opción',
        ajax: {
        url: 'cargar-proveedor',
        dataType: 'json',
        delay: 250,
        processResults: function (data) {
            ...
            results:  $.map(data, function (item) {
                    return {
                        text: item.documento +" -- "+item.nombre_empresa,
                        id: item.idprov
                    }
                })
            };
...
function buscarIngresos(){
    ...
    let filtro=$('#busqueda_ingreso_cmb').val()
    let id_proveedor=$('#proveedor_cmb').val()
    ...
    $.get('filtra-ingreso-bod/'+fecha_inicial+'/'+fecha_final+'/'+filtro+'/'+id_persona, function(data){
        console.log(data)
		...
		if(data.error==false){
			...
			$.each(data.resultado,function(i, item){
                ...
                let empresa_=""
                let tipo_=""
                if(item.ruc=="null" || item.ruc==null){
    
                }else{
                    ruc_=item.ruc
                    empresa_=item.empresa
                    tipo_=item.tipoIngreso
                    console.log(tipo_)
                }
...
function imprimir(id, bodega){
    ...
    $.get("reporte-ingreso-bod-gral/"+id+"/"+bodega, function(data){
        console.log(data)
        vistacargando("")
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
