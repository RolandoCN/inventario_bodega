...
function EditarPaquete(idpaq){
    ...
    $.get("editar-paquete-cirugia/"+idpaq, function(data){
        vistacargando("")
        if(data.error==true){
        ...
        $('#area').val(data.resultado.area).trigger('change.select2')
...
$("#form_paquete").submit(function(e){
    ...
    if(AccionFormPaquete=="R"){
        tipo="POST"
        url_form="guardar-paquete-cirugia"
    }else{
        tipo="PUT"
        url_form="actualizar-paquete-cirugia/"+idPaqueteEditar
    }
...
function llenar_tabla_paquete(){
    ...
    $.get("listado-paquete-cirugia/", function(data){
        ...
        if(data.error==false){
            ...
            $('#tabla_paquete').DataTable({
                ...
                "rowCallback": function( row, data, index ) {
                    ...
                    $('td', row).eq(3).html(`
                                            <button type="button" class="btn btn-primary btn-xs" onclick="EditarPaquete('${data.idpaquete_cirugia }')">Editar</button>

                                            <button type="button" class="btn btn-success btn-xs" onclick="Detalle('${data.descripcion }','${data.idpaquete_cirugia }')">Detalle</button>

                                            <button type="button" class="btn btn-danger btn-xs" onclick="EliminarPaquete('${data.idpaquete_cirugia }')">Eliminar</button>
...
function EliminarPaquete(id){
    if(confirm('¿Quiere eliminar el registro?')){
        vistacargando("")
        $.get("eliminar-paquete-cirugia/"+id, function(data){
            vistacargando("")
            if(data.error==true){
                alertNotificar(data.mensaje,"error");
                return;   
...
function Detalle(descripcion,id_paqute, abiertaModal=null){
    ...
    $.get("detalle-paquete-cirugia/"+id_paqute, function(data){
        console.log(data)
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
        ...
        if(data.error==false){
            ...
            $('#tabla_detalle_paq').DataTable({
                ...
                data: data.resultado,
                columns:[
                        {data: "id_detalle_paquetes_cirugia"},
                        {data: "id_detalle_paquetes_cirugia" },
                        {data: "cantidad" },
                        {data: "id_detalle_paquetes_cirugia" },
                ...
                "rowCallback": function( row, data, index ) {
                    ...
                    $('td', row).eq(3).html(`
                        ...
                        <button type="button" class="btn btn-primary btn-xs" onclick="editarDetallPaq(${data.id_detalle_paquetes_cirugia })"><i class="fa fa-edit"></i></button>            
                                        
                        <a onclick="eliminarDetalle(${data.id_detalle_paquetes_cirugia })" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i>  </a>
...
$("#form_registro_detalle_paq").submit(function(e){
    ...
    if(DetalleAccionForm=="R"){
        tipo="POST"
        url_form="guardar-detalle-paquete-cirugia"
    }else{
        tipo="PUT"
        url_form="actualizar-detalle-paquete-cirugia/"+idDetallePaqEditar
    }
...
function editarDetallPaq(iddetalle_paq){
    vistacargando("m","Espere por favor")
    $.get("editar-detalle-paq-cirugia/"+iddetalle_paq, function(data){
        vistacargando("")
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
            return;   
...
function eliminarDetalle(iddetalle){
    if(confirm('¿Quiere eliminar el registro?')){
        vistacargando("")
        $.get("eliminar-detalle-paq-cirugia/"+iddetalle, function(data){
            vistacargando("")
            if(data.error==true){
                alertNotificar(data.mensaje,"error");
                return;   
