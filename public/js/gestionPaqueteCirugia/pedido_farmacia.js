...
function detalle_paquete(id,descripcion,tipo){
    ...
    $.get(UrlSistema+"/valida-paquete-cirugia/"+id+"/"+cantidad_paq_ingr, function(data){
...
$("#form_pedido_bodega").submit(function(e){
    ...
    if(lugar=="CO"){
        url_form_="guardar-pedido-cirugia"
    }else if(lugar=="CQ"){
        url_form_="guardar-pedido-cirugia"
    }else{
        alertNotificar("Paquete no asociado a bodega")
        return
    }
   
    if(AccionForm=="R"){
        tipo="POST"
        url_form=UrlSistema+"/"+url_form_
    }else{
        tipo="PUT"
        url_form=UrlSistema+"/"+url_form_
    }
