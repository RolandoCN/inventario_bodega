...
function buscarPedidos(){
    ...
    $.get('filtra-pedido-bod-farmacia/'+fecha_inicial+'/'+fecha_final, function(data){
        console.log(data)
...
function Detalle(idcomprobante, idbodega, desc, secue, solicita, area, fecha, bodega, idusuaSoli){
    if(idbodega==31){
        ...
        // paquete centro quirurgico
        DetallePaqueteCQ(idcomprobante, idbodega, desc, secue, solicita, area, fecha, bodega, idusuaSoli)
    }else if(idbodega==33){
        // paquete centro obstetrico
        DetallePaqueteCQ(idcomprobante, idbodega, desc, secue, solicita, area, fecha, bodega, idusuaSoli)
    }else{
...
function DetallePaqueteCQ(idcomprobante, idbodega, desc, secue, solicita, area, fecha, bodega, idusuaSoli){
    ...
    vistacargando("m","Espere por favor")
    $.get("paq-cq-detalle-pedidos-sol/"+idcomprobante+"/"+idbodega, function(data){
        console.log(data)
        vistacargando("")
        if(data.error==true){
            alertNotificar(data.mensaje,"error");
...
function validarPaquete(){
    ...
    if(bodegaSelecc==32){
        //paquete centro quirurgico
        url_paquete="valida-entrega-paquete-cq"
    }else if(bodegaSelecc==33){
        //paquete centro obstetrico
        url_paquete="valida-entrega-paquete-co"
    }else{
        //paquete dialisis
        url_paquete="valida-entrega-paquete"
    }
    swal({
        title: "¿Desea validar los pedidos solicitados?",
        type: "warning",
