...
function buscarPedidos(){
    ...
    $.get('filtra-pedido-receta/'+fecha_inicial+'/'+fecha_final, function(data){
        console.log(data)
		...
		if(data.error==false){
			...
			$.each(data.resultado,function(i, item){
                let estado=""
                if(item.codigo_old=="PedidoAFarm" || item.codigo_old=="Revertido"){
                    estado="Pedido"
                }else{
                    estado="Entregado"
                }
				...
				$('#tabla_pedido').append(`<tr>
                                                ...
                                                <td style="width:7%; text-align:left; vertical-align:middle">
                                                    ...
                                                    <button type="button" data-toggle="tooltip" data-original-title="Entregar" class="btn btn-xs btn-primary" onclick="Detalle('${item.idcomprobante}','${item.idbodega}','${item.descripcion}','${item.secuencial}','${item.solicita}','${item.area}','${item.fecha_hora}','${item.nombre_bodega}','${item.id_usuario_ingresa}')"><i class="fa fa-shopping-cart"></i></button>
...
function DetalleOtros(idcomprobante, idbodega, desc, secue, solicita, area, fecha, bodega, idusuaSoli){
    ...
    $.get("detalle-pedidos-farm/"+idcomprobante+"/"+idbodega, function(data){
        ...
        let bodega_restar=""
        if(idbodega==6){
            bodega_restar="BODEGA FARMACIA GENERAL DE MEDICAMENTOS"
        }else{
            bodega_restar="BODEGA FARMACIA DIALISIS DE MEDICAMENTOS"
        }
        $('.bodega_detalle').html(bodega_restar)
        ...
        if(data.datosReceta!=0){
            ...
            if(data.datosReceta.acompanante==
                "null null null null"){
                $('#secc_acomp').hide()
